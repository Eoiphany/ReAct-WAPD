"""注释
命令:
1. `python -m Autobs.pretrain_policy -v single --network-type surrogate`
2. `python -m Autobs.pretrain_policy -v single --network-type rmnet --model-path /abs/path/to/RMNet.pt`

参数含义:
- `AUTOBS_PMNET_SOURCE / AUTOBS_PMNET_WEIGHTS`: 可选，覆盖默认 SurrogateModel 代码与权重路径。
...
- `python -m Autobs.pretrain_policy`: 策略学习与评估脚本会通过本文件按指定surrogate类型加载模型推理。
"""
