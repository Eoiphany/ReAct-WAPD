...
- 本文件只保留 策略学习实验真正使用的地图读取、数据集子集选择、动作掩码与指标计算。
"""
...
from Autobs.surrogate_adapter import infer_surrogate
...
def load_module_state(module_state_path: str | Path) -> dict[str, Any]:
    """注释
    功能: 从磁盘加载模块状态字典。
    输入: `module_state_path`为权重文件路径。
    输出: 标准化后的状态字典。
    示例: `state = load_module_state("best_module_state.pt")`。
    时间: 2026-04-26。
    """
...
def apply_module_state(agent, module_state: dict[str, Any]) -> None:
    """注释
    功能: 将模块状态字典按键名兼容规则加载到目标代理对象的模块中。
    输入: `agent`需暴露`get_module()`方法，`module_state`为权重字典。
    输出: 无，副作用是更新目标模块参数。
    示例: `apply_module_state(agent_like, state)`。
    时间: 2026-04-26。
    """
    get_module = getattr(agent, "get_module", None)
    if not callable(get_module):
        raise AttributeError("Agent does not expose get_module(); cannot apply module state")
    module = get_module()
