"""
用途:
  PMNet 模型定义文件，用于 USC 训练与 RadioMap3DSeer 微调/评估。

直接运行命令:
  无。该文件是模型定义模块，供其他脚本导入。

导出对象与参数:
  PMNet(n_blocks, atrous_rates, multi_grids, output_stride)
...
class _Bottleneck(nn.Module):
    def __init__(self, in_ch, out_ch, stride, dilation, downsample):
        ...
        self.shortcut = (
            _ConvBnReLU(in_ch, out_ch, 1, stride, 0, 1, False) if downsample else nn.Identity()
        )
...
class _ImagePool(nn.Module):
    ...
    def forward(self, x):
        ...
        h = self.pool(x)
        h = self.conv(h)
        return F.interpolate(h, size=(height, width), mode="bilinear", align_corners=False)
...
class PMNet(nn.Module):
    def __init__(self, n_blocks, atrous_rates, multi_grids, output_stride):
        ...
        self.layer2 = _ResLayer(n_blocks[0], ch[0], ch[2], strides[0], dilations[0])
        self.layer3 = _ResLayer(n_blocks[1], ch[2], ch[3], strides[1], dilations[1])
        self.layer4 = _ResLayer(n_blocks[2], ch[3], ch[3], strides[2], dilations[2])
        self.layer5 = _ResLayer(n_blocks[3], ch[3], ch[4], strides[3], dilations[3], multi_grids)
...
def build_pmnet(output_stride: int = 16) -> PMNet:
    return PMNet(
        n_blocks=[3, 3, 27, 3],
